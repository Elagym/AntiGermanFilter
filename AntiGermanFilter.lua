...
local germanKeywords = {
    ...
    "ein", "einfach", "für", "geht", "gibt", "gilde", "gilden", "gruppe", "günstig", 
    "haben", "heute", "ich", "ist", "jemand", "kann", "kaufe", "keine", "kleine", "lern", "mit", "nein", 
    "netten", "nicht", "nur", "preis", "rekrutiert", "sicheren", "suche", "suchen", "sucht", "über", "und", 
    "unter", "von", "vorhanden", "waffe", "wann", "weiter", "würg", "ß"
}
...
frame:SetScript("OnEvent", function(self, event, addOnName)
    -- IMPORTANT: Replace "AntiGermanFilter" with your actual folder name
    if addOnName == "AntiGermanFilter" then
        
        -- 1. Initialize the global table if it's completely missing
        if _G["germanFilterDB"] == nil then
            _G["germanFilterDB"] = {}
        end

        -- 2. Initialize the keywords sub-table if missing
        if germanFilterDB.keywords == nil then
            germanFilterDB.keywords = {}
        end

        -- 3. Set the default toggle
        if germanFilterDB.printEnabled == nil then
            germanFilterDB.printEnabled = true
        end

        -- 4. POPULATE: If the DB keywords list is empty, fill it from your local list
        -- We check #germanFilterDB.keywords == 0 to see if it's an empty list
        if #germanFilterDB.keywords == 0 then
            for _, word in ipairs(germanKeywords) do
                table.insert(germanFilterDB.keywords, word)
            end
            print("AntiGermanFilter: Database was empty. Default keywords loaded!")
        else
            print("AntiGermanFilter: Database loaded with " .. #germanFilterDB.keywords .. " keywords.")
        end

        -- Cleanup to prevent the event from firing again unnecessarily
